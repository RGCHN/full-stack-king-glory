...
module.exports = app=>{
    ...
    router.get('/walkthrough/list',async(res,req)=>{
        ...
        //获取单篇图文
        router.get('/walkthrough/:id',async(req,res)=>{
            const data = await Walkthrough.findById(req.params.id).lean();
            data.related = await Walkthrough.find().where({
                categories:{$in:data.categories},
                _id:{$ne:data._id}
            }).limit(2);
            res.send(data);
    ...
    //广告位数据接口
    router.get('/ads/:type',async(req,res)=>{
        const data = await Ad.find().where({
